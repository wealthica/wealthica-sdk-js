...
describe('Wealthica User instance (NodeJS)', () => {
  beforeEach(() => {
    mockNode();
    this.user = Wealthica.init({ clientId: 'test', secret: 'test' }).login('test');
  ...
  describe('.fetchToken()', () => {
    test('should call Wealthica /auth/token endpoint', async () => {
      ...
      expect(this.apiMock.history.post[0].baseURL).toBe('https://api.wealthica.com/v1');
    });
