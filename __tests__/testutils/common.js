...
module.exports.setupResource = function ({ isUser } = {}) {
  beforeEach(() => {
    this.wealthica = Wealthica.init({ clientId: 'test', secret: 'test' });
    this.user = this.wealthica.login('test');

    if (isUser) {
      // For user resources (institutions, transactions), create mock on the user object
      ...
      // For data resources (providers, teams), create mock on the wealthica object
      this.apiMock = new MockAdapter(this.wealthica.api.axiosInstance);
    }
  });
};
...
module.exports.testAutoRefreshBehavior = function ({ isBrowser } = {}) {
  describe('when making API calls', () => {
    beforeEach(() => {
      ...
      userApiMock.onGet('/institutions/test').reply(200, { test: 'data' });
    });
  ...
  describe('.getConnectData()', () => {
    test('should return the correct url and token', async () => {
      ...
      const expectedUrl = isBrowser
        ? 'https://connect.wealthica.com/connect?client_id=test&lang=en&origin=http%3A%2F%2Flocalhost&provider_groups=core&theme=light&providers_per_line=2'
        : 'https://connect.wealthica.com/connect?client_id=test&lang=en&provider_groups=core&theme=light&providers_per_line=2';
    ...
    test('should use `redirectURI` if passed to the method or Wealthica.init()', async () => {
      if (isBrowser) {
        this.user = Wealthica.init({ clientId: 'test', redirectURI: 'http://testuri' }).login();
      ...
      // redirectURI passed to the method call takes precedence over Wealthica.init() option
    ...
    test('should refresh token if the existing one has less than 10 minutes left', async () => {
      ...
      await this.user.fetchToken();
      const { url } = await this.user.getConnectData();
      expect(url).toContain('https://connect.wealthica.com/');
      expect(mockObject.history.post).toHaveLength(2);
    });
